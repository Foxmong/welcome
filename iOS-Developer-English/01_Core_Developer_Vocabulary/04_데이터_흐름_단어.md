...
|---|---|---|---|
| request | req | Swift/iOS 문맥에서 request가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| response | res | Swift/iOS 문맥에서 response가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| payload | pay | Swift/iOS 문맥에서 payload가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| endpoint | end | Swift/iOS 문맥에서 endpoint가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| header | hea | Swift/iOS 문맥에서 header가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| body | bod | Swift/iOS 문맥에서 body가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| decode | dec | Swift/iOS 문맥에서 decode가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| encode | enc | Swift/iOS 문맥에서 encode가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| parse | par | Swift/iOS 문맥에서 parse가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| serialize | ser | Swift/iOS 문맥에서 serialize가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| receive | rec | Swift/iOS 문맥에서 receive가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| publish | pub | Swift/iOS 문맥에서 publish가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
...
* request → response → decode → state update 순서로 말한다.
