...
|---|---|---|---|
| available | ava | Swift/iOS 문맥에서 available가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| deprecated | dep | Swift/iOS 문맥에서 deprecated가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| optional | opt | Swift/iOS 문맥에서 optional가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| required | req | Swift/iOS 문맥에서 required가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| mutable | mut | Swift/iOS 문맥에서 mutable가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| immutable | imm | Swift/iOS 문맥에서 immutable가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| valid | val | Swift/iOS 문맥에서 valid가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| invalid | inv | Swift/iOS 문맥에서 invalid가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| empty | emp | Swift/iOS 문맥에서 empty가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| pending | pen | Swift/iOS 문맥에서 pending가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| failed | fai | Swift/iOS 문맥에서 failed가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| successful | suc | Swift/iOS 문맥에서 successful가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
...
* available/deprecated/optional/required는 Apple 문서와 오류 메시지에서 우선 확인한다.
