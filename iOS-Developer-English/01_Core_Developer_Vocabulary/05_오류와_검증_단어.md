...
|---|---|---|---|
| error | err | Swift/iOS 문맥에서 error가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| failure | fai | Swift/iOS 문맥에서 failure가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| warning | war | Swift/iOS 문맥에서 warning가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| issue | iss | Swift/iOS 문맥에서 issue가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| invalid | inv | Swift/iOS 문맥에서 invalid가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| validate | val | Swift/iOS 문맥에서 validate가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| throw | thr | Swift/iOS 문맥에서 throw가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| throws | thr | Swift/iOS 문맥에서 throws가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| catch | cat | Swift/iOS 문맥에서 catch가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| resolve | res | Swift/iOS 문맥에서 resolve가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| prevent | pre | Swift/iOS 문맥에서 prevent가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| missing | mis | Swift/iOS 문맥에서 missing가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
...
* expected와 actual을 찾으면 오류 원인의 절반을 찾은 것이다.
