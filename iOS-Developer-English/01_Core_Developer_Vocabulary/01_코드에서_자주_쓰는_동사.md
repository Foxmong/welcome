...
| commit | com | Swift/iOS 문맥에서 commit가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
| compare | com | Swift/iOS 문맥에서 compare가 가리키는 역할을 파악한다 | 일반 영어의 기본 의미 |
...
* add: `addUserData()` 형태로 함수 이름을 만들어 읽는다.
* append: `appendUserData()` 형태로 함수 이름을 만들어 읽는다.
* apply: `applyUserData()` 형태로 함수 이름을 만들어 읽는다.
* assign: `assignUserData()` 형태로 함수 이름을 만들어 읽는다.
* bind: `bindUserData()` 형태로 함수 이름을 만들어 읽는다.
* build: `buildUserData()` 형태로 함수 이름을 만들어 읽는다.
* cache: `cacheUserData()` 형태로 함수 이름을 만들어 읽는다.
* cancel: `cancelUserData()` 형태로 함수 이름을 만들어 읽는다.
* capture: `captureUserData()` 형태로 함수 이름을 만들어 읽는다.
* cast: `castUserData()` 형태로 함수 이름을 만들어 읽는다.
* commit: `commitUserData()` 형태로 함수 이름을 만들어 읽는다.
* compare: `compareUserData()` 형태로 함수 이름을 만들어 읽는다.
* compile: `compileUserData()` 형태로 함수 이름을 만들어 읽는다.
* configure: `configureUserData()` 형태로 함수 이름을 만들어 읽는다.
* connect: `connectUserData()` 형태로 함수 이름을 만들어 읽는다.
* convert: `convertUserData()` 형태로 함수 이름을 만들어 읽는다.
* create: `createUserData()` 형태로 함수 이름을 만들어 읽는다.
* decode: `decodeUserData()` 형태로 함수 이름을 만들어 읽는다.
* delete: `deleteUserData()` 형태로 함수 이름을 만들어 읽는다.
* dismiss: `dismissUserData()` 형태로 함수 이름을 만들어 읽는다.
