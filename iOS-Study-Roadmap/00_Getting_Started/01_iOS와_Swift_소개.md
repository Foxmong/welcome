...
* iOS 앱은 Swift 코드와 Apple 프레임워크가 함께 동작해 사용자 경험을 만든다를 Swift 코드로 설명한다.
* 문법이 실제 SwiftUI 화면 상태와 어떻게 연결되는지 이해한다.
* 잘못된 코드와 개선된 코드를 비교해 실무 습관을 만든다.

## 2. 핵심 개념

iOS 앱은 Swift 코드와 Apple 프레임워크가 함께 동작해 사용자 경험을 만든다는 앱에서 데이터를 표현하고 화면 동작을 결정하는 기본 도구다. 초보 단계에서는 “값을 만들고, 조건에 따라 바꾸고, 화면에 보여준다”는 흐름으로 이해하면 된다. 실무에서는 같은 문법도 사용자 입력, 서버 응답, 로딩 상태, 빈 화면 처리와 연결된다.
...
struct IntroView: View {
    let appName = "Todo"
    var body: some View { Text("\(appName) 앱을 SwiftUI로 만듭니다") }
}
```
...
```swift
// 언어와 프레임워크를 같은 것으로 이해함
```

## 9. 개선된 코드

```swift
// Swift는 언어, SwiftUI는 UI 프레임워크로 구분함
```
...
Swift와 SwiftUI를 각각 한 문장으로 설명하라.
