...
* Xcode는 코드 작성, 빌드, 실행, 디버깅, 배포를 한곳에서 하는 도구다를 Swift 코드로 설명한다.
* 문법이 실제 SwiftUI 화면 상태와 어떻게 연결되는지 이해한다.
* 잘못된 코드와 개선된 코드를 비교해 실무 습관을 만든다.

## 2. 핵심 개념

Xcode는 코드 작성, 빌드, 실행, 디버깅, 배포를 한곳에서 하는 도구다는 앱에서 데이터를 표현하고 화면 동작을 결정하는 기본 도구다. 초보 단계에서는 “값을 만들고, 조건에 따라 바꾸고, 화면에 보여준다”는 흐름으로 이해하면 된다. 실무에서는 같은 문법도 사용자 입력, 서버 응답, 로딩 상태, 빈 화면 처리와 연결된다.
...
struct XcodeRunView: View {
    @State private var runCount = 0
    var body: some View { Button("Run 횟수: \(runCount)") { runCount += 1 } }
}
```
...
```swift
// 오류 메시지를 읽지 않고 코드를 계속 바꿈
```

## 9. 개선된 코드

```swift
// 첫 번째 오류 메시지부터 원인을 확인함
```
...
Xcode에서 새 SwiftUI 앱을 만들고 ContentView를 수정하라.
