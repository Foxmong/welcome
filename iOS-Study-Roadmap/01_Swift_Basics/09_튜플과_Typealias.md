...
* 튜플은 여러 값을 임시로 묶고 typealias는 복잡한 타입에 이름을 붙인다를 Swift 코드로 설명한다.
* 문법이 실제 SwiftUI 화면 상태와 어떻게 연결되는지 이해한다.
* 잘못된 코드와 개선된 코드를 비교해 실무 습관을 만든다.

## 2. 핵심 개념

튜플은 여러 값을 임시로 묶고 typealias는 복잡한 타입에 이름을 붙인다는 앱에서 데이터를 표현하고 화면 동작을 결정하는 기본 도구다. 초보 단계에서는 “값을 만들고, 조건에 따라 바꾸고, 화면에 보여준다”는 흐름으로 이해하면 된다. 실무에서는 같은 문법도 사용자 입력, 서버 응답, 로딩 상태, 빈 화면 처리와 연결된다.

## 3. 기본 문법

```swift
let result: (title: String, count: Int) = ("Todo", 3)
...
let response = (message: "성공", code: 200)
let headers: HTTPHeaders = ["Accept": "application/json"]
print(response.message, headers)
```
...
typealias TodoSummary = (done: Int, total: Int)
struct SummaryView: View {
    let summary: TodoSummary = (done: 2, total: 5)
    var body: some View { Text("완료 \(summary.done)/\(summary.total)") }
}
```
...
```swift
let user = ("Mina", 25, true, 4.5)
```

## 9. 개선된 코드

```swift
struct User { let name: String; let age: Int; let isActive: Bool; let rating: Double }
```
