...
* String, Int, Double, Bool은 앱 데이터를 안전하게 표현하는 기본 타입이다를 Swift 코드로 설명한다.
* 문법이 실제 SwiftUI 화면 상태와 어떻게 연결되는지 이해한다.
* 잘못된 코드와 개선된 코드를 비교해 실무 습관을 만든다.

## 2. 핵심 개념

String, Int, Double, Bool은 앱 데이터를 안전하게 표현하는 기본 타입이다는 앱에서 데이터를 표현하고 화면 동작을 결정하는 기본 도구다. 초보 단계에서는 “값을 만들고, 조건에 따라 바꾸고, 화면에 보여준다”는 흐름으로 이해하면 된다. 실무에서는 같은 문법도 사용자 입력, 서버 응답, 로딩 상태, 빈 화면 처리와 연결된다.

## 3. 기본 문법

```swift
let title: String = "할 일"
let count: Int = 3
...
print("\(name) \(age) \(rating) \(isPremium)")
```
...
struct TypeDemoView: View {
    let title: String = "오늘의 할 일"
    @State private var remainingCount: Int = 3
    var body: some View { Text("\(title): \(remainingCount)개 남음") }
}
```
